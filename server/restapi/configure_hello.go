...
//go:generate swagger generate server --target ../../server --name Hello --spec ../../schema/hello.v1.yaml --principal interface{}
...
func configureAPI(api *operations.HelloAPI) http.Handler {
	...
	api.GetHelloHandler = operations.GetHelloHandlerFunc(handler.GetHello)
	api.PostHelloHandler = operations.PostHelloHandlerFunc(handler.PostHello)
	...
	if api.PostHelloHandler == nil {
		api.PostHelloHandler = operations.PostHelloHandlerFunc(func(params operations.PostHelloParams) middleware.Responder {
			return middleware.NotImplemented("operation operations.PostHello has not yet been implemented")
		})
	}
